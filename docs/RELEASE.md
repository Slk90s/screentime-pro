...
> 最后更新：2026-09-20（**v0.9.2 已发布**：日志可追溯性改造（每行版本戳 + `audit` 审计文件 + 保留 14 天 +
> 卸载备份）+ 截图失败全局提示 + 内存·状态修复。发布落点（tag / commit、GitHub/Gitee Release id、资产体积）
> 见 §7 版本历史表末行）
...
**当前仓库的三份 `releaseBody` 已是 v0.9.2 的文案**（v0.7.6 起每版发版时同步改写）。
v0.9.2 的 body 为四段：「#### 🔧 修复」写 **截图失败全局提示**（此前只在设置页可见，快捷键截图失败是静默的）、
**自动分类规则反向覆盖用户禁用项**、3 处监听器未注销、空选区误操作、补齐自 v0.8.0 起缺失的提示条样式；
「#### 🔍 可追溯性（新增）」写**每行版本戳**、**`audit.<date>.log` 用户行为审计**（不含窗口标题）、
保留 3→**14 天** + 运行时每小时清理、**卸载前自动备份日志**到「文档\ScreenTimePro-Logs」；
