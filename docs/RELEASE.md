...
> 最后更新：2026-09-20（**v0.9.2 已发布完成**：日志可追溯性改造（每行版本戳 + `audit` 审计文件 + 保留 14 天 +
> 卸载备份）+ 截图失败全局提示 + 内存·状态修复。发布落点：tag / commit `b5b2ae8`；GitHub Release `392344703`
> 6 资产 / 240.6 MiB；Gitee Release `1155479` 5 附件 / 133.9 MiB，AppImage 106.7 MB 超 Gitee 单文件上限，
> 仅 GitHub 提供）
