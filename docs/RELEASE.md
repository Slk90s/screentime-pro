...
> 最后更新：2026-09-20（**v0.9.4 已发布完成**：macOS 截图授权 UX 重做 + ScreenCaptureKit 引擎 ——
> 失败文案精简为一句话 + 紧凑授权弹窗（2s 轮询翻转「已授权点此重启」）+ 启动预请求授权 +
> macOS 14+ SCK 单帧截图（低版本回落 xcap）；新增权限四件套 IPC（84→**88**）；
> Info.plist 补 `NSScreenCaptureUsageDescription`。发布落点：tag / commit `3395626`；
> CI run `35513505604` 三平台全 success；GitHub Release `392440742` **6 资产 / 252.7 MiB**
> （exe 27.18 / dmg 27.10 / app.tar.gz 25.82 / deb 27.15 / rpm 27.15 / AppImage 106.72 MiB）；
> Gitee Release `1156204` **5 附件 / 134.4 MiB**（AppImage 仅 GitHub）；**Gitee 配额 1002.5 / 1024 MiB
> （余 21.5 MiB）—— 下一版发版前必须先删旧版腾空间**。上一版 v0.9.3：tag / commit `beb17fd`；
