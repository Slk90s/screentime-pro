...
> 最后更新：2026-09-20（**v0.9.3 发版中**：macOS 截图修复版 —— 屏幕录制授权判定由**单信号改双信号**，
> 修「系统设置已勾选、应用仍报无权限」；新增 `reset_screen_capture_permission` IPC（83→**84**）与
> 「重置权限并重启」一键按钮；macOS 截图逻辑独立为 `screenshot/macos.rs`（失败文案按证据分档 +
> 修 `CFRelease` 泄漏）。发布落点待 CI 完成后回填。上一版 v0.9.2：tag / commit `b5b2ae8`；
> GitHub Release `392344703` 6 资产 / 240.6 MiB；Gitee Release `1155479` 5 附件 / 133.9 MiB，
> AppImage 106.7 MB 超 Gitee 单文件上限，仅 GitHub 提供）
