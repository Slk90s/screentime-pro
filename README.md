...
| macOS 启动提示「已损坏 / 无法打开，因为开发者无法验证」 | 未公证（Apple Developer 账号未配置，开源免费版不签名） | ① dmg 内自带 `修复门禁.command`，把 app 拖入「应用程序」后**右键 → 打开**该脚本即可；② 或终端执行 `xattr -d com.apple.quarantine "/Applications/ScreenTime Pro.app"` |
